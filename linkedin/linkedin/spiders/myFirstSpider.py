...
class PublicSpider(scrapy.Spider):
    ...
    def after_fetch(self, response):
        ...
        for link in job_link:
            yield response.follow(url=link, callback=self.parse, meta={'dates_list':dates_list})



    def parse(self, response, **kwargs):
         # Job filters
         NLF = ['senior','consultant', 'engineer','fullstack','full stack',
                'sr','engineering']                  
         
         #Pre-processing Job title for filtering
         job_title = response.css('h1.topcard__title::text').extract()
         clean_title = " ".join(job_title).strip().lower()
          
         #Filtering out 'senior', 'consultant' & 'engineer'
         if not any([irrJob in clean_title for irrJob in NLF]):

                 # Pre-processing postlevel for filtering
                 criterion = response.css('span.description__job-criteria-text::text').extract()
                 clean_criterion = " ".join(crit.strip() for crit in criterion).strip().lower()
                 expLevel = ['internship', 'associate', 'mid-senior','director','executive']

                 # Filtering on expLevel
                 if not any([overExp in clean_criterion for overExp in expLevel]):
                         keywords = ['sql','python','analyst']

                         #filter on job desc keyword
                         job_desc = response.css('div.show-more-less-html__markup ::text').extract()
                         clean_desc_temp = " ".join(job_desc).strip().lower().replace("."," ").replace(","," ")
                         if any([keyword in clean_desc_temp for keyword in keywords]):
                                 #now filtering on yrs. of exp
                                 desc_items_list = response.css('div.show-more-less-html__markup > ul > li ::text').extract()

                                 for item in desc_items_list:
                                      if 'experience' in item:
                                          clean_item = item.replace("-", " ").replace("\'"," ") 
                                          numbers_NA = ['two', 'three',  'four', 'five', 'six', 'seven', 'eight', 'nine', 
                                                        'ten', '2', '3', '4', '5', '6', '7', '8','9','10','15']                        
                                          if any([NAnumber in clean_item for NAnumber in numbers_NA]):                             
                                              return None
...
# =============================================================================
# #---------------
# linkedin Website search filters
# (Python OR Sql) NOT (engineer OR Engineering OR Consultant OR Senior OR 'sr' OR 'co-op' OR fullstack OR 'full stack')

# https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=%28Python%2BOR%2BSql%29%2BNOT%2B%28engineer%2BOR%2BEngineering%2BOR%2BConsultant%2BOR%2BSenior%2BOR%2B%27sr%27%2BOR%2B%27co-op%27%2BOR%2Bfullstack%2BOR%2B%27full%2Bstack%27%29&location=Ontario%2C%2BCanada&geoId=105149290&trk=public_jobs_jobs-search-bar_search-submit&start=75


# 'new grad', 'help desk engineer'/'support engineer'   

 
# "https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?currentJobId=3335051477&geoId=105149290&location=Ontario%2C%20Canada&start={}"
#  #--------               
#          sent_desc = sent_tokenize(job_desc)
#          for sentence in sent_desc:
#              if 'experience' in sentence:
#                  print()
#                  print(sentence)
#                  print()
#                  if any([word in posting['description'].lower() for word in sentence]):
#                     
# 
# 
# =============================================================================
...
#save industry name, and job function to get insights


#etl
# =============================================================================
# 
# in the DB: sort by date
# print then pretty print the postings
# 
# other filters to try: 'new grad'  or 'support engineer'
# 
# then compare the total no. of postings for the three categories.
# Watever has more postings, has more demand, but also, think about the barrier to entry for support eng. 
# also look at the biggest companies that are hiring then see if they have info sessions
# 
# ALSo, you're  focusing too much on technical skills. You gotta show them a hollisitc view of who you are.
# 
# 
# #FIX
# -the css selectors extract lists, so you cant use .lower() on them
# 
# #---------- fine-grained scraping of job desc. page
# 
# ok so the description has paras, if i see " "
#         
# # =============================================================================
# =============================================================================
#         import json
